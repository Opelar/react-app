...
const reducer = createReducer(initialState, {
  [START_INVOKE]: (state, action) =>
    state.set("isFetching", true).set("loaded", false).set("serviceType", action.payload.getIn(["request", "method"])),
  ...
  [INVOKE_OK]: (state, action) => {
    ...
    let loaded = false;
    if (requestCount != null && responseData != null && responseData.size != null) {
